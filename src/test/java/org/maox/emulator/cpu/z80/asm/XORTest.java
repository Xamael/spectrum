...
public class XORTest {
	...
	final private static Logger logger = LoggerFactory.getLogger(XORTest.class);
	...
	public void testExecuteDirect() {
		...
		try {
			/* Valor directo */
			Instruction ins = new XOR(Z80.DIRECT_8);
			ins.setCPU(cpu);
			...
			assertEquals(cpu.getA(), (byte) 0b00101100);
	...
	public void testExecuteHL() {
		...
		try {
			...
			Instruction ins = new XOR(Z80.ADDR_HL);
			ins.setCPU(cpu);
			logger.trace(ins.getAssembly());
			...
			assertEquals(cpu.getA(), (byte) 0b01101010);
	...
	public void testExecuteIX_d() {
		...
		try {
			...
			Instruction ins = new XOR(Z80.IX_D);
			ins.setCPU(cpu);
			logger.trace(ins.getAssembly());
			cpu.setA((byte) 0b10101010);
			int address = 0x00FF;
			...
			assertEquals(cpu.getA(), (byte) 0b01110110);
	...
	public void testExecuteReg() {
		...
		try {
			/* Prueba registro B */
			Instruction ins = new XOR(Z80.REG_B);
			ins.setCPU(cpu);
			logger.trace(ins.getAssembly());
			cpu.setA((byte) 0b10101010);
			logger.trace("Register A: {}h", Hex.byteToHex(cpu.getA()));
			...
			/* Prueba registro D */
			ins = new XOR(Z80.REG_D);
			ins.setCPU(cpu);
			logger.trace(ins.getAssembly());
			cpu.setA((byte) 0b11100111);
			logger.trace("Register A: {}h", Hex.byteToHex(cpu.getA()));
			cpu.setD((byte) 0b11000011);
			logger.trace("Register D: {}h", Hex.byteToHex(cpu.getD()));
			ins.execute();
			logger.trace("A <- A ^ D: {}h", Hex.byteToHex(cpu.getA()));

			assertEquals(cpu.getA(), (byte) 0b00100100);
