...
public class ANDTest {
	...
	final private static Logger logger = LoggerFactory.getLogger(ANDTest.class);
	...
	public void testExecuteDirect() {
		...
		try {
			/* Valor directo */
			Instruction ins = new AND(Z80.DIRECT_8);
			ins.setCPU(cpu);
			...
			assertEquals(cpu.getA(), (byte) 0b11000011);
	...
	public void testExecuteHL() {
		...
		try {
			...
			Instruction ins = new AND(Z80.ADDR_HL);
			ins.setCPU(cpu);
			logger.trace(ins.getAssembly());
			...
			assertEquals(cpu.getA(), (byte) 0b10000000);
	...
	public void testExecuteIX_d() {
		...
		try {
			...
			Instruction ins = new AND(Z80.IX_D);
			ins.setCPU(cpu);
			logger.trace(ins.getAssembly());
			cpu.setA((byte) 0b10101010);
			int address = 0x00FF;
			...
			assertEquals(cpu.getA(), (byte) 0b10001001);
	...
	public void testExecuteReg() {
		...
		try {
			/* Prueba registro B */
			Instruction ins = new AND(Z80.REG_B);
			ins.setCPU(cpu);
			logger.trace(ins.getAssembly());
			cpu.setA((byte) 0b10101010);
			logger.trace("Register A: {}h", Hex.byteToHex(cpu.getA()));
			...
			assertEquals(cpu.getA(), (byte) 0b00000000);

			/* Prueba registro C */
			ins = new AND(Z80.REG_C);
			ins.setCPU(cpu);
			logger.trace(ins.getAssembly());
			cpu.setA((byte) 0b11111111);
			logger.trace("Register A: {}h", Hex.byteToHex(cpu.getA()));
			cpu.setC((byte) 0b11000011);
